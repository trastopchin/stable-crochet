...
bool key_down(igl::opengl::glfw::Viewer &viewer, unsigned char key, int modifier)
{
    ...
    else if (key == '3')
    {
        viewer.data().clear();
        viewer.data().set_mesh(V, F);
        viewer.data().set_edges(V_cat, E_cat, gold);
        viewer.data().set_points(V_cat, gold);
        viewer.core().align_camera_center(V, F);
    }
...
void arap_based_helper(igl::opengl::glfw::Viewer &viewer)
{
